// ads.js
// Handles advertising calculator, email, and PDF generation
...
class Ads {
  ...
  calcTotal() {
    ...
    if (start && end && !isNaN(start) && !isNaN(end)) {
      ...
      if (days > 0 && days < STATION_CONFIG.prices.surchargeThreshold) {
        ...
        if (this.surchargeEl) this.surchargeEl.style.display = 'none';
      }
    }
  ...
  genPDF() {
    ...
    // Show element temporarily for pdf
    pdfEl.style.display = 'block';
    html2pdf().from(pdfEl).save('RockScot_Contract.pdf').then(() => {
